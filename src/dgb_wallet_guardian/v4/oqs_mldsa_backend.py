...
class OqsMlDsaBackend:
    """liboqs-python backed ML-DSA signer/verifier for Guardian Wallet evidence.
    ...
    ``ML-DSA-65``. It signs and verifies Guardian Wallet component verdict
    evidence bytes only; it does not sign transactions and does not broadcast.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if mechanism != OQS_ML_DSA_MECHANISM:
            raise GuardianWalletV4RealCryptoBackendError("Shield v4 policy.v1 requires OQS ML-DSA-65")
    ...
    def _load_oqs(self) -> Any:
        ...
        try:
            ...
            raise GuardianWalletV4RealCryptoBackendUnavailable(
                "liboqs-python import oqs is required for ML-DSA"
            ) from exc
        except Exception as exc:
            self._raise_oqs_error("import", exc)

    def _raise_oqs_error(self, operation: str, exc: Exception) -> NoReturn:
        raise GuardianWalletV4RealCryptoBackendError(f"OQS ML-DSA {operation} failed closed") from exc
    ...
    def _require_mechanism_enabled(self) -> Any:
        ...
        if self.mechanism not in enabled:
            raise GuardianWalletV4RealCryptoBackendUnavailable("OQS ML-DSA-65 mechanism is not enabled")
        return oqs
    ...
    def _require_expected_binary_length(
        ...
        *,
        ...
        if not isinstance(details, dict):
            raise GuardianWalletV4RealCryptoBackendError("OQS ML-DSA details must be a mapping")
        expected = details.get(detail_key)
        if expected is None:
            return
        if isinstance(expected, bool) or not isinstance(expected, int) or expected <= 0:
            raise GuardianWalletV4RealCryptoBackendError(f"OQS ML-DSA {detail_key} must be a positive integer")
        if len(value) != expected:
            raise GuardianWalletV4RealCryptoBackendError(f"{field} byte length must be {expected} for OQS ML-DSA-65")
    ...
    def sign_message(self, *, algorithm: str, private_key_reference: str, message: bytes) -> str:
        """Sign Guardian Wallet Shield v4 evidence bytes using OQS ML-DSA-65."""

        if algorithm != OQS_ML_DSA_ALGORITHM:
            raise GuardianWalletV4RealCryptoBackendUnavailable("OQS backend only supports Shield v4 ml-dsa")
    ...
    def verify_signature(self, *, algorithm: str, public_key: str, message: bytes, signature: str) -> bool:
        """Verify a Guardian Wallet Shield v4 ML-DSA signature using OQS ML-DSA-65."""
        ...
        try:
            ...
            if not isinstance(verified, bool):
                raise GuardianWalletV4RealCryptoBackendError("OQS ML-DSA verify must return bool")
            return verified
