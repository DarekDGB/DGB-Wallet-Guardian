...
from dgb_wallet_guardian.v4.signing import COMPONENT_VERDICT_DOMAIN
from dgb_wallet_guardian.v4.trust_profile import require_non_empty_str, require_positive_int, require_supported_algorithm
...
class GuardianWalletV4RealCryptoBackendError(ValueError):
    """Base fail-closed error for Guardian Wallet Shield v4 real crypto backend wiring."""


class GuardianWalletV4RealCryptoBackendUnavailable(GuardianWalletV4RealCryptoBackendError):
    """Raised when a required production backend or algorithm is unavailable."""


class GuardianWalletV4RealCryptoMaterialError(GuardianWalletV4RealCryptoBackendError):
    """Raised when deterministic TEST-ONLY material reaches the real backend."""


class GuardianWalletV4RealCryptoBackend(Protocol):
    """Minimal production crypto backend contract for Guardian Wallet v4 evidence.
...
def _wrap_value_error(operation: str, callback: Callable[[], _T]) -> _T:
    try:
        ...
        raise GuardianWalletV4RealCryptoBackendError(f"{operation} failed closed: {exc}") from exc


def _require_real_non_empty_str(value: Any, *, field: str) -> str:
    if not isinstance(value, str) or not value:
        raise GuardianWalletV4RealCryptoBackendError(f"{field} must be non-empty string")
    if value != value.strip():
        raise GuardianWalletV4RealCryptoBackendError(f"{field} must not contain surrounding whitespace")
    return value
...
def _require_hash(value: Any, *, field: str) -> str:
    ...
    if len(clean) != 64:
        raise GuardianWalletV4RealCryptoBackendError(f"{field} must be 64-character sha256 hex")
    try:
        int(clean, 16)
    except ValueError as exc:
        raise GuardianWalletV4RealCryptoBackendError(f"{field} must be sha256 hex") from exc
    if clean != clean.lower():
        raise GuardianWalletV4RealCryptoBackendError(f"{field} must be lowercase sha256 hex")
    return clean
...
def _reject_test_only_text(value: str, *, field: str) -> None:
    ...
    if any(marker in clean for marker in _TEST_ONLY_MARKERS) or any(clean.startswith(prefix) for prefix in _TEST_ONLY_PREFIXES):
        raise GuardianWalletV4RealCryptoMaterialError(f"{field} must not contain test-only material")
...
def encode_binary_signature_material(raw: bytes, *, field: str = "signature") -> str:
    ...
    if not isinstance(raw, bytes) or not raw:
        raise GuardianWalletV4RealCryptoBackendError(f"{field} bytes must be non-empty")
...
def decode_binary_signature_material(encoded: Any, *, field: str = "signature") -> bytes:
    ...
    if not clean.startswith(REAL_SIGNATURE_ENCODING_PREFIX):
        raise GuardianWalletV4RealCryptoBackendError(f"{field} must use b64u encoding")
    body = clean[len(REAL_SIGNATURE_ENCODING_PREFIX) :]
    if not body:
        raise GuardianWalletV4RealCryptoBackendError(f"{field} b64u payload must be non-empty")
    if "=" in body:
        raise GuardianWalletV4RealCryptoBackendError(f"{field} b64u payload must be unpadded")
    if set(body) - _BASE64URL_ALPHABET:
        raise GuardianWalletV4RealCryptoBackendError(f"{field} b64u payload is invalid")
    try:
        decoded = base64.urlsafe_b64decode(body + "=" * (-len(body) % 4))
    except (binascii.Error, ValueError) as exc:
        raise GuardianWalletV4RealCryptoBackendError(f"{field} b64u payload is invalid") from exc
    if not decoded:  # pragma: no cover - base64url cannot reach this after the body check.
        raise GuardianWalletV4RealCryptoBackendError(f"{field} b64u payload must decode to non-empty bytes")
    return decoded
...
def build_real_crypto_signature_input(
    *,
    ...
    """Build the exact production-signature message bytes for Guardian Wallet evidence.

    The signed payload hash is already domain-separated over the canonical Guardian
    ...
    if clean_domain not in _ALLOWED_DOMAIN_TAGS:
        raise GuardianWalletV4RealCryptoBackendError(
            "domain_tag must be the Guardian Wallet Shield v4 component signing domain"
        )
...
def _require_backend_supports_algorithm(backend: GuardianWalletV4RealCryptoBackend, algorithm: str) -> None:
    try:
        supported = tuple(getattr(backend, "supported_algorithms", ()))
    except Exception as exc:
        raise GuardianWalletV4RealCryptoBackendError("real crypto backend algorithm discovery failed closed") from exc
    if algorithm not in supported:
        raise GuardianWalletV4RealCryptoBackendUnavailable("real crypto backend does not support required algorithm")


def _call_backend_sign(
    backend: GuardianWalletV4RealCryptoBackend,
    *,
    algorithm: str,
    ...
    try:
        ...
        raise GuardianWalletV4RealCryptoBackendError("real crypto backend sign failed closed") from exc
...
def _call_backend_verify(
    ...
    *,
    ...
    try:
        ...
        raise GuardianWalletV4RealCryptoBackendError("real crypto backend verify failed closed") from exc
    if not isinstance(verified, bool):
        raise GuardianWalletV4RealCryptoBackendError("real crypto backend verify must return bool")
    return verified
...
def build_signature_entry_with_real_backend(
    *,
    ...
    """Build a Guardian Wallet Shield v4 signature entry using a real backend."""
...
def _validated_key_fields(key: dict[str, Any], *, algorithm: str, key_id: str, key_version: int) -> dict[str, Any]:
    if not isinstance(key, dict):
        raise GuardianWalletV4RealCryptoBackendError("registry key must be dict")
    if set(key.keys()) != _REGISTRY_KEY_FIELDS:
        raise GuardianWalletV4RealCryptoBackendError("registry key fields must match required schema")
    role = _require_real_non_empty_str(key.get("role"), field="role")
    if role != COMPONENT_ROLE:
        raise GuardianWalletV4RealCryptoBackendError("registry key role mismatch")
    ...
    if (key_algorithm, key_key_id, key_key_version) != (algorithm, key_id, key_version):
        raise GuardianWalletV4RealCryptoBackendError("signature entry does not match registry key")
...
def verify_signature_entry_with_real_backend(
    ...
    *,
    ...
    """Verify one Guardian Wallet Shield v4 signature entry with a production backend."""

    if not isinstance(entry, dict):
        raise GuardianWalletV4RealCryptoBackendError("signature entry must be dict")
    if set(entry.keys()) != _SIGNATURE_ENTRY_FIELDS:
        raise GuardianWalletV4RealCryptoBackendError("signature entry fields must match required schema")
...
def make_real_crypto_signature_verifier(
    ...
    """Adapt a real crypto backend to the existing Guardian Wallet bundle verifier callback."""
