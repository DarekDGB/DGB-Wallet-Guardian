...
from dgb_wallet_guardian.contracts.v3_2_lock import SUPPORTED_DECISIONS, SUPPORTED_EVIDENCE_FAMILIES, SUPPORTED_REASON_IDS
from dgb_wallet_guardian.v4 import CANONICALIZATION_PROFILE, COMPONENT_ID, CONTRACT_VERSION, POLICY_VERSION, VERDICT_SCHEMA_VERSION
from dgb_wallet_guardian.v4.signing import SignatureVerifier, signed_payload_hash, verify_signature_bundle
from dgb_wallet_guardian.v4.trust_profile import require_non_empty_str, require_positive_int, validate_freshness_window
...
def build_signed_crypto_verdict_envelope(*, unsigned_payload: dict[str, Any], signature_bundle: dict[str, Any]) -> dict[str, Any]:
    if set(unsigned_payload.keys()) != REQUIRED_UNSIGNED_VERDICT_FIELDS:
        raise ValueError("unsigned Guardian Wallet v4 verdict payload fields must match required schema")
    return {
        **unsigned_payload,
...
def validate_crypto_verdict_envelope(
    ...
    *,
    ...
    if not isinstance(verdict, dict):
        raise ValueError("Guardian Wallet v4 verdict must be dict")
    if set(verdict.keys()) != REQUIRED_SIGNED_VERDICT_FIELDS:
        raise ValueError("Guardian Wallet v4 verdict fields must match required schema")
