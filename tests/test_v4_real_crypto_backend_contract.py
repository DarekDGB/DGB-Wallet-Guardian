...
from dgb_wallet_guardian.v4.real_crypto_backend import (
    ...
    GuardianWalletV4RealCryptoMaterialError,
...
from dgb_wallet_guardian.v4.signing import COMPONENT_VERDICT_DOMAIN, build_signature_bundle, verify_signature_bundle
...
PAYLOAD_HASH = "a" * 64
PUBLIC_KEY_BYTES = b"guardian-wallet-real-ml-dsa-public-key"
...
class HierarchySignFailureBackend(FakeRealBackend):
    def sign_message(self, *, algorithm: str, private_key_reference: str, message: bytes) -> str:
        raise GuardianWalletV4RealCryptoBackendUnavailable("backend hierarchy sign failure")


class HierarchyVerifyFailureBackend(FakeRealBackend):
    def verify_signature(self, *, algorithm: str, public_key: str, message: bytes, signature: str) -> bool:
        raise GuardianWalletV4RealCryptoBackendUnavailable("backend hierarchy verify failure")


def real_key(*, algorithm: str = "ml-dsa", public_key: str | None = None) -> dict[str, Any]:
    return {
        "role": COMPONENT_ROLE,
        "key_id": f"shield_component_guardian_wallet-{algorithm}-v1",
        "key_version": 1,
        "algorithm": algorithm,
...
def test_v48g_r4_guardian_wallet_real_crypto_signature_input_is_frozen_to_guardian_wallet_domain() -> None:
    encoded = build_real_crypto_signature_input(
        algorithm="ml-dsa",
        standard_profile=FIPS204_ML_DSA_65_PROFILE,
        domain_tag=COMPONENT_VERDICT_DOMAIN,
        signed_payload_hash=PAYLOAD_HASH,
        key_id="shield_component_guardian_wallet-ml-dsa-v1",
        key_version=1,
    )
    ...
    assert encoded == (
        ...
        "shield_component_guardian_wallet-ml-dsa-v1\n"
        "1"
    ).encode("utf-8")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"key_id": ""}, "key_id"),
        ({"key_id": " shield_component_guardian_wallet-ml-dsa-v1"}, "surrounding whitespace"),
        ({"key_version": 0}, "key_version"),
        ({"key_version": True}, "key_version"),
    ],
)
def test_v48g_r4_guardian_wallet_real_crypto_signature_input_rejects_ambiguous_values(
    ...
    base.update(kwargs)
    with pytest.raises((GuardianWalletV4RealCryptoBackendError, ValueError), match=match):
        build_real_crypto_signature_input(**base)  # type: ignore[arg-type]


def test_v48g_r4_guardian_wallet_real_crypto_signer_builds_b64u_entry_without_test_fallback() -> None:
...
def test_v48g_r4_guardian_wallet_real_crypto_signer_rejects_private_material_backend_gap_and_malformed_backend_output() -> None:
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="b64u"):
        build_signature_entry_with_real_backend(
            algorithm="ml-dsa",
            standard_profile=FIPS204_ML_DSA_65_PROFILE,
            domain_tag=COMPONENT_VERDICT_DOMAIN,
...
def test_v48g_r4_guardian_wallet_real_crypto_backend_wrapper_catches_native_exceptions() -> None:
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="algorithm discovery") as algorithm_error:
        build_signature_entry_with_real_backend(
            algorithm="ml-dsa",
            standard_profile=FIPS204_ML_DSA_65_PROFILE,
            domain_tag=COMPONENT_VERDICT_DOMAIN,
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="sign failed closed") as sign_error:
        build_signature_entry_with_real_backend(
            algorithm="ml-dsa",
            standard_profile=FIPS204_ML_DSA_65_PROFILE,
            domain_tag=COMPONENT_VERDICT_DOMAIN,
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="verify failed closed") as verify_error:
        verify_signature_entry_with_real_backend(
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="verify must return bool"):
        verify_signature_entry_with_real_backend(
            signature_for_key(real_key()),
            real_key(),
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendUnavailable, match="hierarchy sign failure"):
        build_signature_entry_with_real_backend(
            algorithm="ml-dsa",
            standard_profile=FIPS204_ML_DSA_65_PROFILE,
            domain_tag=COMPONENT_VERDICT_DOMAIN,
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendUnavailable, match="hierarchy verify failure"):
        verify_signature_entry_with_real_backend(
            signature_for_key(real_key()),
            real_key(),
...
def test_v48g_r4_guardian_wallet_real_crypto_verifier_accepts_real_backend_and_rejects_tamper() -> None:
    key = real_key()
...
def test_v48g_r4_guardian_wallet_real_crypto_verifier_adapter_matches_guardian_wallet_bundle_callback_shape() -> None:
...
def test_v48g_r4_guardian_wallet_real_crypto_verifier_fails_closed_on_test_key_material_and_key_mismatch() -> None:
    test_public_key = real_key(public_key="TEST-ONLY-PUBLIC-shield_component_guardian_wallet-ml-dsa-v1")
    with pytest.raises(GuardianWalletV4RealCryptoMaterialError, match="test-only"):
    ...
    test_key_id["key_id"] = "test-shield_component_guardian_wallet-ml-dsa-v1"
    test_entry = signature_for_key(real_key())
    test_entry["key_id"] = "test-shield_component_guardian_wallet-ml-dsa-v1"
    with pytest.raises(GuardianWalletV4RealCryptoMaterialError, match="test-only"):
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="role"):
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="registry key"):
        verify_signature_entry_with_real_backend(signature_for_key(real_key()), wrong_algorithm_key, backend=FakeRealBackend())

    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="dict"):
...
def test_v48g_r4_guardian_wallet_real_crypto_verifier_rejects_bad_entry_and_backend_gap_before_verify() -> None:
    key = real_key()
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="dict"):
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="signature entry fields"):
        verify_signature_entry_with_real_backend(extra_entry, key, backend=FakeRealBackend())

    extra_key = real_key()
    extra_key["authority"] = "forbidden"
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="registry key fields"):
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="public_key"):
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="domain_tag"):
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="lowercase"):
        verify_signature_entry_with_real_backend(bad_hash, key, backend=FakeRealBackend())


def test_v48g_r4_guardian_wallet_real_binary_encoding_helpers_are_strict() -> None:
    ...
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="bytes"):
        encode_binary_signature_material(b"", field="signature")
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="b64u"):
        decode_binary_signature_material("abc", field="signature")
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="non-empty"):
        decode_binary_signature_material("b64u:", field="signature")
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="unpadded"):
        decode_binary_signature_material("b64u:YWJj=", field="signature")
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="invalid"):
        decode_binary_signature_material("b64u:****", field="signature")
    with pytest.raises(GuardianWalletV4RealCryptoBackendError, match="invalid"):
