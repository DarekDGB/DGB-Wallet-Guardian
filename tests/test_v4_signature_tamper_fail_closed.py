...
from dgb_wallet_guardian.v4.signing import COMPONENT_VERDICT_DOMAIN, verify_test_only_signature, verify_signature_bundle
from dgb_wallet_guardian.v4.trust_profile import ACTIVE, build_test_trust_profile

from tests.test_v4_crypto_verdict_contract import HASH_A, NOT_AFTER, NOT_BEFORE, VERIFY_AT, signed_verdict


def test_guardian_wallet_v4_tampered_signature_fails_closed() -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda entry: entry.__setitem__("key_id", "wrong-key"), "trusted Guardian Wallet key"),
...
def test_guardian_wallet_v4_signature_entry_mutations_fail_closed(mutator, match: str) -> None:
    verdict = signed_verdict()
...
def test_guardian_wallet_v4_duplicate_algorithm_fails_closed() -> None:
    verdict = signed_verdict()
...
def test_guardian_wallet_v4_revoked_key_and_key_window_fail_closed() -> None:
    verdict = signed_verdict()
